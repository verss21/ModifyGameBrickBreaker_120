...
class Game(tk.Frame):
    ...
    def add_bricks(self):
        for x in range(5, self.width - 5, 75):
            for y_offset in range(3):
                hits = max(1, 3 - y_offset)
                self.add_brick(x + 37.5, 50 + y_offset * 20, hits)
    ...
    def game_loop(self):
        ...
        if num_bricks == 0:
            ...
            self.lives -= 1
            if self.lives < 0:
                self.draw_text(300, 200, "Game Over! Press R to restart.")
                self.canvas.bind("<r>", lambda _: self.reset_game())
            else:
